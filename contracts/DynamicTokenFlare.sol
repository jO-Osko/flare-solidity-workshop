...
import { IIFtso } from "@flarenetwork/flare-periphery-contracts/flare/ftso/ftso/interface/IIFtso.sol";
...
library FTSOPrices {
    ...
    function getFtsoRegistry(IPriceSubmitter priceSubmitter) internal view returns (IFtsoRegistry) {
        return IFtsoRegistry(address(priceSubmitter.getFtsoRegistry()));
    }

    function getCurrentPricesBySymbols(IPriceSubmitter priceSubmitter, string[] memory _symbols) internal view returns (IFtsoRegistry.PriceInfo[] memory) {
        IFtsoRegistry ftsoRegistry = getFtsoRegistry(priceSubmitter);
        return ftsoRegistry.getCurrentPricesBySymbols(_symbols);
    }
